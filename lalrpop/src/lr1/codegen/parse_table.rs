...
impl<'ascent, 'grammar, W: Write> CodeGenerator<'ascent, 'grammar, W, TableDriven<'grammar>> {
    ...
    fn write_error_recovery_fn(&mut self) -> io::Result<()> {
        ...
        let parameters = vec![
            ...
            format!("{p}states: &mut ::std::vec::Vec<i32>", p = self.prefix),
            format!(
        ...
        rust!(
            self.out,
            "let {p}action = {p}ACTION[({p}state * {} + {}) as usize];",
            actions_per_state,
            actions_per_state - 1,
            p = self.prefix
        );
        ...
        rust!(
            self.out,
            "let {p}error_action = {p}ACTION[({p}recover_state * {} + {}) as usize];",
            actions_per_state,
            actions_per_state - 1,
            p = self.prefix
        );
    ...
    fn write_accepts_fn(&mut self) -> io::Result<()> {
        ...
        let actions_per_state = self.grammar.terminals.all.len();
        let parameters = vec![
            format!("{p}error_state: i32", p = self.prefix),
            format!("{p}states: & [i32]", p = self.prefix),
